...
function ModalUploadProduction({ title, callback, onSuccess }) {
  ...
  const handleUploadProductions = useCallback(
    async (data: Production) => {
      ...
      try {
        ...
        const result = await client_productions_api.uploadProductions(formData);
        ...
        toast({
          ...
          description: "Las producciones se han subido correctamente.",
        });

        form.reset();
        setOpen(false);
      } catch (error: any) {
        console.error("Error subiendo producciones:", error);

        toast({
          variant: "destructive",
          title: "Error al subir producciones",
          description: error.message || "Hubo un error al subir las producciones.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleAccountChange = (clientId: string) => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[400px]">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleUploadProductions)} className="grid gap-2">
...
export default ModalUploadProduction;